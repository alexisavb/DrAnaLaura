...
- **SEO**: meta description/keywords, canonical URL, Open Graph tags, and a `Physician` JSON-LD structured-data block (name, address, phone, specialty, `openingHoursSpecification`) — keep all of these in sync if practice details (address, phone, hours) change elsewhere on the page.
