...
- **SEO**: meta description/keywords, canonical URL, Open Graph tags, and a `Physician` JSON-LD structured-data block (name, address, phone, specialty, `openingHoursSpecification`) — keep all of these in sync if practice details (address, phone, hours) change elsewhere on the page. `sitemap.xml` and `robots.txt` live at the repo root (both served as-is by GitHub Pages, no config needed); bump `<lastmod>` in the sitemap when the page changes meaningfully — it's manual, nothing regenerates it.
